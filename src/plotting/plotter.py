...
def plot_example_errors(images_test, correct ,cls_true,class_names, cls_pred, smooth=True):
    ...
    num_test = len(class_names)


    images = images_test[incorrect]

    # Get the predicted classes for those images.
    cls_pred2 = cls_pred[incorrect]


    # Get the true classes for those images.
    cls_true2 = cls_true[incorrect]
    ...
    plot_images(images, cls_true2 ,class_names, cls_pred=cls_pred2, smooth=True)
...
def montage_filters(W):
    ...
    return m
def plot_montage(batch_of_images , size_of_figure = ( 12,12  )):
    batch_of_images = batch_of_images[:,0,0,:,:,:]
    #print(np.mean(batch_of_images))
    batch_of_images  = ( batch_of_images + 1.0 ) / np.max(batch_of_images)
    #print(np.std(batch_of_images ))
    mont = montage(batch_of_images)
