...
class Inception:
    ...
    def _create_feed_dict(self, image_path=None, image=None):
        ...
        return feed_dict
    def get_first_layer(self, image_path=None,image=None):
        ...
        W = self.graph.get_tensor_by_name('conv/Conv2D:0')       
        first_layer = self.session.run(   W , feed_dict=feed_dict)
        return first_layer
        
    def get_gradient_first_layer(self, image_path=None,image=None):
        # Create a feed-dict for the TensorFlow graph with the input image.
        feed_dict = self._create_feed_dict(image_path=image_path, image=image)

        # Execute the TensorFlow session to get the predicted labels.
        #W = self.graph.get_tensor_by_name('conv_2/conv2d_params:0')
        feature= self.graph.get_tensor_by_name('conv/Conv2D:0')       
        #first_layer = self.session.run(   W , feed_dict=feed_dict)
        gradient = tf.gradients(tf.reduce_max(feature, 3), self.resized_image)
        grad_cal= self.session.run(   gradient , feed_dict=feed_dict)
        print(len(grad_cal))
        return grad_cal
    def get_gradients_any_layer(self, image_path=None,image=None ,name_of_layer='conv/Conv2D:0'):
        # Create a feed-dict for the TensorFlow graph with the input image.
        feed_dict = self._create_feed_dict(image_path=image_path, image=image)
        ...
        for i in range( shape_of_tensor[-1] ):
            gradient = tf.gradients(feature[:,:,:,i] ,self.resized_image   )            
            #gradient = tf.gradients(tf.reduce_mean(feature[:,:,:,i]), self.resized_image)            
            grad_cal= self.session.run(   gradient , feed_dict=feed_dict)
            grad_list.append(grad_cal)
        #print(len(grad_cal))
        return grad_list
    def deep_dream(self,image_path=None, image=None ,name_of_layer='conv/Conv2D:0'):
        feed_dict = self._create_feed_dict(image_path=image_path, image=image)
        original_image = feed_dict[self.tensor_name_input_jpeg] 
        layer = self.graph.get_tensor_by_name(name_of_layer)  
        shape_of_tensor_layer = tf.shape(layer).eval(session=self.session,feed_dict=feed_dict)
        print("original",shape_of_tensor_layer)
        
        sum_of_layer =tf.reduce_mean(layer, 3)
        #sum_of_layer =tf.reduce_mean(layer)
        shape_of_tensor_layer_sum =tf.shape(sum_of_layer).eval(session=self.session,feed_dict=feed_dict)
        print("sum",shape_of_tensor_layer_sum)
        gradient = tf.gradients(sum_of_layer, self.resized_image )        
        grad_cal= self.session.run(   gradient , feed_dict=feed_dict)        
        ...
        n_iterations = 50
        # Think of this as our learning rate.  This is how much of the gradient we'll add to     the               input     
        step = 1.0
        ...
        for it_i in range(n_iterations):
            ...
            this_res = self.session.run(   gradient   , feed_dict=feed_dict)
            # Let's normalize it by the maximum activation
            this_res /= (np.max(np.abs(this_res)) + 1e-8)
            
            #print("this_cal" ,this_res.shape )
            #print("original image1",original_image.shape )
            #print("original image2",img_copy.shape )
            # Then add it to the input image
            img_copy=np.add(  this_res[0,0,:,:,:]*step , img_copy )
